...
export default class RichMouseHandler extends BasicMouseHandler {
  ...
  onMouseDown(grid: DataGrid, event: MouseEvent): void {
    ...
    if (handle === 'left' || handle === 'right') {
      ...
      const type: 'column-resize' = 'column-resize';
    ...
    if (handle === 'top' || handle === 'bottom') {
      // Set up the resize data type.
      const type: 'row-resize' = 'row-resize';
    ...
    if (region === 'row-header' && handle === 'grab') {
      // Set up the resize data type.
  ...
  onWheel(grid: DataGrid, event: WheelEvent): void {
    ...
    switch (event.deltaMode) {
      ...
      case 1: // DOM_DELTA_LINE
        dx *= grid.defaultSizes.columnWidth;
        dy *= grid.defaultSizes.rowHeight;
        break;
  ...
  protected pressData: Private.PressData | null;
